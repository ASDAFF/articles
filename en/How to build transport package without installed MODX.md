# How to build transport package without installed MODX

Create own CMP not simple task for newbie, but hardened developer sometimes face challenges too. Newbie will be scared and experienced developer will solve the problem :).

In this article, I talk about an ability to create and build transport packages for MODX without installing and setting MODX. The upper mediate level is required, as in some cases you will need to solve strange behavior and other troubles, but worth it.

Details below.

Times ago, when MODX Revolution only appeared, when it was in the early beta-version, developers not knew yet, how they can work with MODX and how they can create extras for it. Well, except the core team that pored over CMS. And team, I should say, provided in the system ability to create and build packages, that can be installed via Installer in the future. But as time goes and requirements to the packages and their building are the others for now.

## Copy-paste is evil, but not always
...
### Creating a namespace and adding it into the package
...
### Adding attributes to the package
...
Ask questions, will try to answer.

P.S. Do not miss, [star Slackify on the  GitHub](https://github.com/Alroniks/modx-slackify), please.
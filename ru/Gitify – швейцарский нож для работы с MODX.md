...
А теперь разберемся с build. Эта команда по задумке должна пройтись по всем файлам из папки _data и попробовать создать объекты MODX на основании сохраненной в этих файлах информации.

Т.е. если у нас в папке _data/chunks/ есть файл header.html, значит gitify попробует создать чанк с именем header, а в качестве значений полей для объекта чанка будут использоваться значения полей из самого файла (не забываем, у нас там данные в yaml-формате).

> Стоит заметить, что работать с сайтами в кодировке cp1251 не получится, так как сам формат YAML поддерживает только Unicode (если верить спецификации).

Как и в случае команды extract, собирать можно не весь сайт, а только какую-то его часть, достаточно лишь указать категорию элементов для сборки. Например, собрать только чанки:

```bash
gitify build chunks
```

Помимо этого у команды build относительно много дополнительных параметров и их комбинаций. 

#### —skip-clear-cache

По умолчанию gitify после сборки очищает системный кеш MODX, но возможно в некоторых случаях того делать не нужно. Эта настройка отключает автоматическую очистку кеша при сборке. 
...
=== написать про разрешение конфликтов id ===
...
=== написать тут про этот баг https://github.com/modxcms/revolution/pull/12669 и рассказать как лечить ===
...
=== написать обязательно об gitignore ===  и git
